...
read -p 'Email for LetsEncrypt account [xx@xx.xx]: ' letsencrypt_email
read -p 'Mail server domain name [xx.xx.xx]: ' mail_server_url

# Check all variables have a value
if [[ -z ${mail_server_url} ]] || [[ -z ${letsencrypt_email} ]]
then
    echo Run script again and insert all values. Do not miss any value.
else
...
zmprov ms ${host} zimbraReverseProxyMailMode redirect
zmprov ms ${host} zimbraMailMode https
zmprov ms ${host} zimbraReverseProxySSLToUpstreamEnabled TRUE
zmprov ms ${host} zimbraMailClearTextPasswordEnabled FALSE
zmprov ms ${host} zimbraImapCleartextLoginEnabled FALSE
zmprov gs ${host} zimbraReverseProxyImapStartTlsMode only
zmprov ms ${host} zimbraPop3CleartextLoginEnabled FALSE
zmprov gs ${host} zimbraReverseProxyPop3StartTlsMode only
zmprov ms ${host} zimbraMtaTlsAuthOnly TRUE
zmlocalconfig -e postfix_smtp_tls_security_level=may
zmprov ms ${host} zimbraMtaTlsSecurityLevel may
zmprov gs ${host} zimbraAuthLdapStartTlsEnabled TRUE
EOF
